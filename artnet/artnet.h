...
#ifndef ARTNET_EXTERN
#ifdef MSW
#define ARTNET_EXTERN __declspec(dllexport) extern
#else
#define ARTNET_EXTERN extern
#endif /* MSW */
#endif
...
ARTNET_EXTERN int ARTNET_ADDRESS_NO_CHANGE;
...
ARTNET_EXTERN artnet_node artnet_new(const char *ip, int verbose);
ARTNET_EXTERN int artnet_setoem(artnet_node vn, uint8_t hi, uint8_t lo);
ARTNET_EXTERN int artnet_setesta(artnet_node vn, char hi, char lo);
ARTNET_EXTERN int artnet_set_bcast_limit(artnet_node vn, int limit);
ARTNET_EXTERN int artnet_start(artnet_node n);
ARTNET_EXTERN int artnet_read(artnet_node n, int timeout);
ARTNET_EXTERN int artnet_stop(artnet_node n);
ARTNET_EXTERN int artnet_destroy(artnet_node n);

int artnet_join(artnet_node vn1, artnet_node vn2);

// handler functions
// these need to be cleaned up into a generic interface
ARTNET_EXTERN int artnet_set_handler(artnet_node vn,
  artnet_handler_name_t handler,
  int (*fh)(artnet_node n, void *pp, void *d),
  void* data);
ARTNET_EXTERN int artnet_set_dmx_handler(artnet_node vn,
  int (*fh)(artnet_node n, int port, void *d),
  void *data);
ARTNET_EXTERN int artnet_set_program_handler(artnet_node vn,
  int (*fh)(artnet_node n, void *d),
  void *data);
ARTNET_EXTERN int artnet_set_firmware_handler(artnet_node vn,
  int (*fh)(artnet_node n, int ubea, uint16_t *data, int length, void *d),
  void *data);
ARTNET_EXTERN int artnet_set_rdm_handler(artnet_node vn,
  int (*fh)(artnet_node n, int address, uint8_t *rdm, int length, void *d),
  void *data);
ARTNET_EXTERN int artnet_set_rdm_initiate_handler(artnet_node vn,
  int (*fh)(artnet_node n, int port, void *d),
  void *data);
ARTNET_EXTERN int artnet_set_rdm_tod_handler(artnet_node vn,
  int (*fh)(artnet_node n, int port, void *d),
  void *data);

// send functions
ARTNET_EXTERN int artnet_send_poll(artnet_node n,
  const char *ip,
  artnet_ttm_value_t talk_to_me);
ARTNET_EXTERN int artnet_send_poll_reply(artnet_node n);
ARTNET_EXTERN int artnet_send_dmx(artnet_node n,
  int port_id,
  int16_t length,
  const uint8_t *data);
ARTNET_EXTERN int artnet_raw_send_dmx(artnet_node vn,
  uint8_t uni,
  int16_t length,
  const uint8_t *data);
ARTNET_EXTERN int artnet_send_address(artnet_node n,
  artnet_node_entry e,
...
ARTNET_EXTERN int artnet_send_input(artnet_node n,
  artnet_node_entry e,
  uint8_t settings[ARTNET_MAX_PORTS]);
ARTNET_EXTERN int artnet_send_firmware(artnet_node vn,
  artnet_node_entry e,
  int ubea,
  uint16_t *data,
  int length,
  ...
  void *user_data);
ARTNET_EXTERN int artnet_send_firmware_reply(artnet_node vn,
  artnet_node_entry e,
  artnet_firmware_status_code code);

// rdm functions
ARTNET_EXTERN int artnet_send_tod_request(artnet_node vn);
ARTNET_EXTERN int artnet_send_tod_control(artnet_node vn,
  uint8_t address,
  artnet_tod_command_code action);
ARTNET_EXTERN int artnet_send_tod_data(artnet_node vn, int port);
ARTNET_EXTERN int artnet_send_rdm(artnet_node vn,
  uint8_t address,
  uint8_t *data,
  int length);
ARTNET_EXTERN int artnet_add_rdm_device(artnet_node vn,
  int port,
  uint8_t uid[ARTNET_RDM_UID_WIDTH]);
ARTNET_EXTERN int artnet_add_rdm_devices(artnet_node vn,
  int port,
  uint8_t *uid,
  int count);
ARTNET_EXTERN int artnet_remove_rdm_device(artnet_node vn,
  int port,
  uint8_t uid[ARTNET_RDM_UID_WIDTH]);

// recv functions
ARTNET_EXTERN uint8_t *artnet_read_dmx(artnet_node n, int port_id, int *length);

// state changing functions
ARTNET_EXTERN int artnet_set_node_type(artnet_node n, artnet_node_type type);
ARTNET_EXTERN int artnet_set_short_name(artnet_node vn, const char *name);
ARTNET_EXTERN int artnet_set_long_name(artnet_node n, const char *name);

//port manipulation functions
ARTNET_EXTERN int artnet_set_port_type(artnet_node n,
                                int id,
                                artnet_port_settings_t settings,
                                artnet_port_data_code data);
ARTNET_EXTERN int artnet_set_port_addr(artnet_node n,
                                int id,
                                artnet_port_dir_t dir,
                                uint8_t addr);
ARTNET_EXTERN int artnet_set_subnet_addr(artnet_node n, uint8_t subnet);
ARTNET_EXTERN int artnet_get_universe_addr(artnet_node n,
                                    int id,
                                    artnet_port_dir_t dir);

//node list functions
ARTNET_EXTERN artnet_node_list artnet_get_nl(artnet_node n);
ARTNET_EXTERN artnet_node_entry artnet_nl_first(artnet_node_list nl);
ARTNET_EXTERN artnet_node_entry artnet_nl_next(artnet_node_list nl);
ARTNET_EXTERN int artnet_nl_get_length(artnet_node_list nl);

// misc
ARTNET_EXTERN int artnet_dump_config(artnet_node n);
ARTNET_EXTERN int artnet_get_config(artnet_node n, artnet_node_config_t *config);
ARTNET_EXTERN artnet_socket_t artnet_get_sd(artnet_node n);
ARTNET_EXTERN int artnet_set_fdset(artnet_node vn, fd_set *fdset);

ARTNET_EXTERN char *artnet_strerror();
